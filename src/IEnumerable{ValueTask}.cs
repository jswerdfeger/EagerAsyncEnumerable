...
/// <summary>
/// Provides extensions on <see cref="IEnumerable{ValueTask}"/> to obtain an eager-loading
/// implementation of <see cref="IAsyncEnumerable{bool}"/> and
/// <see cref="IAsyncEnumerator{bool}"/>.
/// </summary>
public static class IEnumerableValueTask
{
	/// <summary>
	/// Returns this <see cref="IEnumerable{ValueTask}"/> as an eager-loading enumerable, which,
	...
	/// running when you call this, so there is no benefit to using this.</para>
	/// </remarks>
	public static IAsyncEnumerable<bool> AsEagerEnumerable(this IEnumerable<ValueTask> enumerable)
	{
	...
	/// <see cref="ValueTask"/>. The <paramref name="cancellationToken"/> you supply must be the
	/// same one you used to create all the tasks, or it will only have the capability to cancel
	/// between tasks, not during.
	/// </para>
	/// </remarks>
	public static IAsyncEnumerator<bool> GetEagerEnumerator(this IEnumerable<ValueTask> enumerable,
	...
	{
		private readonly IEnumerable<ValueTask> _enumerable;

		public EagerEnumerable(IEnumerable<ValueTask> enumerable)
		{
	...
	{
		private readonly IEnumerator<ValueTask> _source;
		protected override ValueTask ReadAllTask { get; }

		private int _queued;

		public EagerEnumerator(IEnumerable<ValueTask> source, CancellationToken cancellationToken)
