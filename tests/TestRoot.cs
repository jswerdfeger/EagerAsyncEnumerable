...
[TestClass]
public abstract class TestRoot<TSource, T>
{
	protected record struct ProcessResults(List<T> List, int Time);

	protected class MyException : Exception { }

	protected abstract TSource Producer(int count, int delay, int exceptionIndex = -1,
		CancellationToken cancellationToken = default);

	protected abstract IAsyncEnumerable<T> AsEagerEnumerable(TSource source);

	// We have a consumer for both your TSource as well as the eager enumerable to confirm the
	// results are the same.
	protected abstract ValueTask<ProcessResults> Consumer(TSource source, int delay);

	protected async ValueTask<ProcessResults> Consumer(IAsyncEnumerable<T> source, int delay,
	...
	public async Task AssertCancellation(int count, int producerDelay, int consumerDelay)
	{
		...
		try
		{
			var actual = await Consumer(AsEagerEnumerable(source), consumerDelay, -1,
	...
	public async Task AssertComplete(int count, int producerDelay, int consumerDelay)
	{
		...
		var actual = await Consumer(AsEagerEnumerable(source), consumerDelay);
	...
	public async Task AssertConsumerException(int count, int producerDelay, int consumerDelay)
	{
		...
		try
		{
			var actual = await Consumer(AsEagerEnumerable(source), consumerDelay, count / 2);
		}
		catch (MyException)
		{
			return;
	...
	public async Task AssertDisposal(int count, int producerDelay, int consumerDelay)
	{
		...
		await foreach (var item in AsEagerEnumerable(source))
		{
	...
	/// <summary>Asserts no exceptions if producer is empty.</summary>
	[TestMethod, Timeout(30000)]
	public async Task AssertNoEntries()
	{
		int count = 0, producerDelay = 100, consumerDelay = 100;
		var source = Producer(count, producerDelay);
		await Consumer(AsEagerEnumerable(source), consumerDelay);
	}
